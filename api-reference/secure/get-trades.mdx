...
description: "Cursor-paginated history of trades the merchant initiated through secure sell, secure buy and quick buy; end-user client trades are not included on this endpoint."
---

Returns trades the merchant initiated through `/secure/sell`, `/secure/buy` or `/secure/buy/quick`. End-user (client) trades are **not** included on this endpoint: clients can fetch their own trade history via `/client/trades` with a client token.
