...
| `bans.gameBans` | number | Game bans on record. Blocks CS2 deposits; Rust is unaffected |
...
| `2` | CS2Banned | Can trade Rust items but not CS2 items. Also returned when `bans` shows a VAC or game ban: Steam refuses CS2 offers from such accounts even though the inventory still lists the items as tradable, and a CS2 deposit fails with `USER_NOT_TRADEABLE` |
