...
| 7 | `USER_NOT_TRADEABLE` | User's Steam account can't trade. For CS2 this includes a VAC or game ban on record (Steam refuses to move the items, even though the inventory still lists them as tradable); the error `details` carry `vacBanned`, `gameBans` and `economyBan`. Rust deposits are not affected by VAC or game bans |
