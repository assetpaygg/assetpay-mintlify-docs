...
| `externalId` | string | No | Your unique tracking ID (max 128 chars). Must be unique per request; every trade of a split basket carries it (see Split baskets). |
| `isInstant` | boolean | No | Request instant credit during the hold period (default `false`). Fails with `INSTANT_DEPOSITS_DISABLED` if instant deposits are off for your account. |
...
A single deposit request can become more than one trade. Items are routed by how we take them in (our own bots or a deposit partner), and each route is its own trade with its own Steam offer, status and webhooks. The response carries the primary trade (the highest-value split) at the top level and the others under `relatedTrades`.

Every trade of the basket carries your `externalId`. Secondary trades also carry `splitOf` with the primary trade's id. Lookups and cancels by `externalId` resolve to the primary trade; address a secondary by its own `id`. Credit each trade on its own `completed` event: the totals are per trade, never per basket.
...
| 7 | `USER_NOT_TRADEABLE` | User's Steam account can't trade. For CS2 this includes a VAC ban on record (Steam refuses to move the items, even though the inventory still lists them as tradable); the error `details` carry `vacBanned`, `gameBans` and `economyBan`. Game bans do not block trading, and Rust deposits are not affected by VAC bans |
...
| 1622 | `INSTANT_DEPOSITS_DISABLED` | Instant deposits are disabled for your account and the request set `isInstant: true` (omit the field or send `false`) |
