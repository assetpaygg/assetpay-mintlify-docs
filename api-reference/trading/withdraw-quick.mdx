...
Initiates a **quick** withdrawal. Instead of naming specific listings, you pass an `itemId`, a per-unit price ceiling, and an amount, and AssetPay buys the cheapest available listings at or below your ceiling and delivers them to the user via Steam trade offer.
...
| `externalIds` | array | No | One reference of your own per requested unit (each 1–128 chars), applied to the pre-filled rows in order. Must hold **exactly** `amount` entries, all unique. Accepted in place of the item id by both [cancel](/api-reference/trading/cancel-withdraw-item) endpoints. On Rust the rows are merged into one (see [Response](#response)), which carries no `externalId`. |
| `delivery` | string | No | `"standard"` or `"instant"`. Defaults to `"standard"`. `"instant"` is CS2-only. When instant quick withdrawals are switched off server-side, an `"instant"` request is accepted and quietly placed as `"standard"`; the rows then carry `delivery: "standard"`. |
...
- `items` contains **one pre-filled row per requested unit** (so `amount: 3` returns 3 rows), each carrying the ceiling `price` and `delivery`, the same minimal shape as a standard buy item. The catalog `itemId` is used as each row's placeholder `id` until it is **bound** to a real listing. On Rust, rows for the same item are merged, so you get a single row whose `amount` is the number of units, with no `externalId`.
...
<Warning>
Quick withdrawals require your merchant account to have an active callback URL and an API secret configured. Calling this endpoint without them fails immediately and no trade is created: `MERCHANT_NO_CALLBACK_URL` (1705), `MERCHANT_CALLBACK_URL_DISABLED` (1706) or `MERCHANT_NO_API_SECRET` (1707).
</Warning>
...
| 1001 | `VALIDATION_FAILED` | Invalid request body (HTTP 400): `amount` out of range (1–200), `maxPrice` ≤ 0, neither `itemId` nor `marketHashName`, `externalIds` not exactly `amount` unique entries, `phase` / instant delivery / `autoCancel` on Rust, or other invalid input. `error.fields` maps each invalid field to its reason |
