...
A deposit request can split into several trades, one per intake route, and each trade sends its own callbacks. Every trade of the basket carries your `externalId`; the secondary ones also carry `splitOf` with the primary trade id. Key your records on the trade `id`, and credit each trade on its own `completed`.
...
| `reverted` | The deposit was reversed, usually during the hold. Reverse any `preCredit` + `pendingCredit` already credited. Check `revertedBy`. |
