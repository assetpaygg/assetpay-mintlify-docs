...
description: "How Signicat PictureID verification works and what it unlocks: the verification flow, checking status, multi-account linking, and the full trust cascade."
---
...
Open `captureUrl` in the user's browser. After they complete the capture, they are redirected to AssetPay's own `/verification/complete` page, not to your site, so poll the status endpoint to learn the result.

<Warning>
Only one verification process can be active at a time per user. If a process started within the last 30 minutes still has no result (`CREATED`), you'll get a `VERIFICATION_IN_PROGRESS` error. Wait for it to finish, or for 30 minutes to pass since it started, before starting a new one.
</Warning>
...
| Status | Meaning |
|--------|---------|
| `CREATED` | Verification started, capture URL generated, no result yet. An abandoned process stays `CREATED` |
...
`latestProcess` shows the `ACCEPTED` process when there is one, even if a later attempt exists.
...
2. On a second account, you call `POST /client/verification/start` with the same email address. This stores the email, but it also starts a new verification process on that account, which blocks another `/start` for 30 minutes
...
When accounts are linked to the same identity, **trust penalties cascade across all of them**. If one linked account has a CS2 deposit reversed, the trust factor is destroyed on every account tied to that identity.
...
| Code | Key | When |
|------|-----|------|
| 2300 | `VERIFICATION_NOT_AVAILABLE` | Verification is not enabled yet (HTTP 503) |
| 2301 | `VERIFICATION_ALREADY_VERIFIED` | Account is already at level 1 (HTTP 400) |
| 2302 | `VERIFICATION_IN_PROGRESS` | A process started in the last 30 minutes has no result yet (HTTP 400) |
| 2305 | `VERIFICATION_ACCOUNT_CAP_REACHED` | Too many accounts linked to this identity (HTTP 400) |
| 2310 | `VERIFICATION_EMAIL_REQUIRED` | Email is needed for linking (pass it in `/start`) (HTTP 400) |
| 2312 | `VERIFICATION_LINK_FAILED` | No identity matches the email, or the identity is banned (HTTP 400) |

A banned identity is not an error on `/start`: the capture completes and the process ends `REJECTED`.
