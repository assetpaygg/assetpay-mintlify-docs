...
**Active exposure** is the sum of `preCredit` from all of the user's currently pending trades (trades in `initiated`, `active`, or `hold` status). This prevents a single user from leveraging the same collateral pool across many simultaneous deposits.
