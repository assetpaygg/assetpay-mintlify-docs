...
description: "How instant CS2 deposit credit works: the preCredit and pendingCredit split, collateral sources, verification caps, and handling the reversal callbacks."
---
...
<Info>
Rust trades have **no trade protection period**. Rust deposits are never split: they carry `preCredit` equal to `totalPrice` and `pendingCredit` `0`, and are credited in full when the trade reaches `completed` (right after the user accepts), regardless of any settings described on this page.
</Info>

## How It Works

When a user deposits CS2 skins with `isInstant: true`, AssetPay splits the deposit value into two parts:
...
**Active exposure** is the sum of `preCredit` from all of the user's currently pending trades (trades in `initiated`, `pending`, `active`, or `hold` status). This prevents a single user from leveraging the same collateral pool across many simultaneous deposits.
...
Instant credit is off by default. To request it, set `isInstant: true` when you initiate a deposit (this fails with `INSTANT_DEPOSITS_DISABLED` if instant deposits are off for your account):
...
Omit `isInstant` (or send `false`) to skip instant credit entirely. The full amount will be held until `completed`.
...
| `reverted` | Reverse only what you have already credited: `preCredit` during `hold`, `preCredit` + `pendingCredit` after `completed` |

On CS2, if `isInstant` was `false` or the user had zero collateral, `preCredit` will be `0` and the full amount arrives on `completed`. Rust has no `hold`, so a Rust deposit's `preCredit` equals `totalPrice` and nothing is credited before `completed`: credit `totalPrice` there.

<Warning>
Always handle `reverted` callbacks. A deposit is normally reversed during the hold period (reverse only the `preCredit` already given; `pendingCredit` was never credited), but it can also be reversed after `completed` (reverse both `preCredit` and `pendingCredit`). Failing to handle reversals creates a balance discrepancy.
</Warning>

## Trust and Reversals

Reversals carry real consequences. When a CS2 deposit is reverted:
